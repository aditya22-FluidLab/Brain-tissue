...
def clip_closed_surface(surf, normal='x', origin=None, tolerance=1e-6, inplace=False, progress_bar=False):
...
def extract_surface(mask, grid, mesh_reduction_factor, taubin_smooth_iter, filename=None):
    ...
    assert not np.isnan(surf.points).any()
...
def extract_surf_id(obj_id, mesh_reduction_factor, taubin_smooth_iter, filename):
    surf = extract_surface(padded == obj_id, grid, mesh_reduction_factor, taubin_smooth_iter, filename=filename)
    if surf:
	return obj_id
    else:
	return None

def extract_cell_meshes(img, cell_labels, resolution, mesh_reduction_factor=10, taubin_smooth_iter=5, write_dir=None, ncpus=1):
    global padded
    global grid
    padded = np.pad(img, 1)
    ...
    args = [(obj_id, mesh_reduction_factor, taubin_smooth_iter, f"{write_dir}/{obj_id}.ply") for obj_id in cell_labels]
...
def create_balanced_csg_tree(surface_files):
	...
	return {
            "operation": "union",
            "left": create_balanced_csg_tree(surface_files[:int(n/2)]),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--infile", help="path to image data", type=str)
    parser.add_argument("--outdir", help="directory for output", type=str, default="output")
    parser.add_argument("--ncpus", help="number of cores, default 1", type=int, default=1)
    ...
    img = img_grid["values"].reshape(dims, order="F")
    ...
    if "roimask" in img_grid.array_names:
        roimask = img_grid["roimask"].reshape(dims - np.array([1, 1, 1]), order="F")
        roipadded = np.pad(roimask, 1)
        grid = pv.ImageData(dimensions=roipadded.shape, spacing=resolution, origin=(0, 0, 0))
        roisurf = extract_surface(roipadded, grid, mesh_reduction_factor=10, taubin_smooth_iter=5)
 ...
 mesh_files = [outdir / f"{cid}.ply" for cid in surfs if cid]
    ...
    with open(outdir / "csgtree.json", "w") as outfile:
        json.dump(csg_tree, outfile)

    print(f"Surface meshes and CSG tree saved in {outdir}")


# Merge all individual meshes into one
merged = pv.merge([pv.read(str(f)) for f in mesh_files if f.exists()])
merged.save(outdir / "merged_cells.ply")
print(f"Merged mesh saved to {outdir / 'merged_cells.ply'}")
